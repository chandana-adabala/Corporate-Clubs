...
namespace CorporateClubs.Services.Services
{
    public class ClubsService : IClubs
    {
        ...
        public bool MakeNCancelRequest(int clubID, int userID)
        {
            using (var _context = new ModelContext())
            {
                try
                {
                    ...
                    c.JoiningDate = DateTime.Now;
                    c.IsPersonBlock = false;
                    c.RowModifiedBy = userID;
                    c.RowModifiedOn = DateTime.Now;
                    c.Role = "User";
                    _context.SaveChanges();
                }
                catch
        ...
        public bool RemoveUser(int clubID, int userID, int currentUserID)
        {
            using (var _context = new ModelContext())
            {
                try
                {
                    ...
                    c.RowDeletedBy = currentUserID;
                    c.RowModifiedBy = currentUserID;
                    c.RowModifiedOn = DateTime.Now;
                    c.IsRequested = false;
                    c.IsFavouriteClub = false;
                    c.IsPersonBlock = false;
                    _context.SaveChanges();
                }
                catch
        ...
        public  bool AddUserToPublicClub(int clubID,int userID)
        {
            using (var _context = new ModelContext())
            {
                try
                {
                    if (!Addmember(clubID, userID, userID, false))
                        throw new Exception();
        ...
        //public void GetOnlyClubMembersListofUser(int userID)
        //{
        //    using (var _context = new ModelContext())
        //    {
        //        return _context.ClubMembers.where()
        //    }
        //}
        ...
        public List<ClubMembersList> GetClubMembersListofUser(int userID)
        {
            using (var _context = new ModelContext())
            {
                return _context.Clubs.GroupJoin(_context.ClubMembers.Where(c => c.UserID == userID&& c.RowDeletedBy!=null), c => c.ClubID, m => m.ClubID, (clubs, members) => new { clubs, members }).SelectMany(z => z.members.DefaultIfEmpty(), (club, member) => new ClubMembersList {clubs=club.clubs,members=member,count=_context.ClubMembers.Where(c=>c.ClubID==club.clubs.ClubID).Count()
